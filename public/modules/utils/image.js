...
module.exports = class {
    ...
    static preVerify = async (
        ...
        const isSupportFileSize = async (fileSize) => {
            ...
            if (fileSize > maxSize || fileSize < minSize) {
                throw 'file size not supported';
            }
        };

        const isSupportResolution = async (type, width, height) => {
            let maxResolution = type === 'webp' ? 16383 : 65535;
            if (width <= 0 || height <= 0 || width >= maxResolution || height >= maxResolution) {
    ...
    static postVerify = async (
        inputFilePath,
        outputFilePath,
        { checkEqualResolution = true, checkSSIM = true, checkFileSize = true }
    ) => {
        ...
        const check_ssim = async (input, output) => {
            ...
            const image1 = await loadImage(input);
            const image2 = await loadImage(output);
            const result = ImageSSIM.compare(image1, image2);

            if (result.ssim <= 0.8) {
                console.log('ssim: ', result.ssim);
                console.error('ssim abnormality');
                throw 'file ssim abnormality';
            }
        };
        ...
        if (checkSSIM) {
            ...
            await check_ssim(input_img, output_img);
        }
        if (checkFileSize) {
